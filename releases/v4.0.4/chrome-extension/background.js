/**
 * Background Service Worker v4.0.0
 * 
...
console.log('[学习报告插件] Service Worker v4.0.3 启动');
  ...
  return {
    data: list.map(item => {
      ...
      return {
        ...
        studentName: item.studentName || item.name || item.stuName || '',
...
function buildCSV(students) {
  ...
  lines.push([
    ...
    '学习效果','掌握度','首次答对率%','引导后答对率%','首次答对数','引导答对数','引导提问总数',
    '练习情况','练习正确率%','有记录练习数','做对数','做错数',
    '错题本收录','总练习数(系统)','错题率%','知识点总数','已完成知识点数','完成率%','总互动次数'
  ].map(escapeCSV).join(','));

  for (let i = 0; i < students.length; i++) {
    const s = students[i];
    const row = [
      i + 1, s.name, s.overallTag, s.diagnosis, s.quadrant,
      s.courseName, s.lessonName,
      s.label, s.rate != null ? s.rate : '', s.totalAsk, s.totalAns,
      s.focusRating || '', s.focusAnswer, s.overOther,
      s.masteryLabel, s.masteryRating, s.firstRate != null ? s.firstRate : '',
      s.guideRate != null ? s.guideRate : '', s.firstCorrectTotal, s.guideCorrectTotal, s.guideNumTotal,
      s.exerLabel, s.exerRate != null ? s.exerRate : '',
      s.exerTotalRecorded, s.exerCorrectCount, s.exerWrongCount,
      s.wrongNum, s.questionNum, s.wrongRate + '%',
      s.knowledgeCount, s.completedKnowledge, s.completionRate + '%', s.interactNum
    ];
  ...
  for (const lesson of summary) {
    ...
    if (lesson.avgExercise !== null) {
      lines.push(['平均练习正确率', lesson.avgExercise + '%'].map(escapeCSV).join(','));
    }
    lines.push('');
    lines.push('【学生排名】按回答率升序');
    lines.push(['排名','姓名','回答率%','掌握度','综合标签','一句话诊断'].map(escapeCSV).join(','));
    for (const [idx, s] of lesson.ranking.entries()) {
      lines.push([idx + 1, s.name, (s.rate != null ? s.rate : '') + '%', s.masteryRating, s.overallTag, s.diagnosis].map(escapeCSV).join(','));
    }
    lines.push('');
    if (lesson.problemStudents.length > 0) {
      lines.push('【问题学生名单】');
      lines.push(['姓名','问题类型','关键指标','建议动作'].map(escapeCSV).join(','));
      for (const p of lesson.problemStudents) {
        ...
        lines.push([p.name, ptype, `回答率 ${p.rate}%`, action].map(escapeCSV).join(','));
      }
    } else {
      lines.push('【无问题学生】✅');
    }
