/**
 * 调课助手 v4.0 - Popup 逻辑
 * 功能：Token提取复制、学员信息簿导入、自然语言解析、姓名/手机号匹配、历史日志、批量并发
 */
...
// Tab 切换
const elTabToken = document.getElementById('tab-token');
const elTabMain = document.getElementById('tab-main');
const elTabHistory = document.getElementById('tab-history');
const elPanelToken = document.getElementById('panel-token');
const elPanelMain = document.getElementById('panel-main');
const elPanelHistory = document.getElementById('panel-history');
// V4 Token
const elTokenStatusCard = document.getElementById('token-status-card');
const elTokenDetail = document.getElementById('token-detail');
const elTokenPreview = document.getElementById('token-preview');
const elTokenExpire = document.getElementById('token-expire');
const elTokenUser = document.getElementById('token-user');
const elBtnCopyToken = document.getElementById('btn-copy-token');
const elBtnRefreshToken = document.getElementById('btn-refresh-token');
// 历史日志
...
async function executeTasks() {
  ...
  // 执行任务（并发池，3个一组）
  const CONCURRENCY = 3;
  const TASK_INTERVAL = 500; // 每个任务间隔500ms（比原来的2秒快4倍）

  let pendingTasks = taskList.filter((t) => t.status === TASK_STATUS.PENDING);
  ...
  while (pendingTasks.length > 0) {
    ...
    // 取出一批任务（最多 CONCURRENCY 个）
    const batch = pendingTasks.splice(0, CONCURRENCY);

    // 并发执行这一批
    const promises = batch.map(async (task, batchIdx) => {
      // 错开启动时间，避免同时发请求
      if (batchIdx > 0) await delay(batchIdx * TASK_INTERVAL);
...
const HISTORY_STORAGE_KEY = 'tiaokeHistory';
const MAX_HISTORY_RECORDS = 500; // 最多保存500条
...
async function loadHistory() {
  try {
    ...
    if (Array.isArray(result[HISTORY_STORAGE_KEY])) {
      historyRecords = result[HISTORY_STORAGE_KEY];
    }
  } catch (e) {
    console.warn('[调课助手] 加载历史记录失败', e);
  }
}
...
async function saveHistory() {
  try {
    ...
    if (historyRecords.length > MAX_HISTORY_RECORDS) {
      historyRecords = historyRecords.slice(-MAX_HISTORY_RECORDS);
    }
    await chrome.storage.local.set({ [HISTORY_STORAGE_KEY]: historyRecords });
  } catch (e) {
    console.warn('[调课助手] 保存历史记录失败', e);
  }
}
...
function renderHistory() {
  if (historyRecords.length === 0) {
    elHistoryContainer.innerHTML = '<div class="history-empty">暂无历史记录</div>';
    return;
  }
  ...
  for (const date of Object.keys(grouped).sort((a, b) => b.localeCompare(a))) {
    ...
    for (const r of grouped[date]) {
      ...
      const name = r.matchedName ? `${r.matchedName}(${r.userId})` : r.userId;
      const statusClass = r.success ? 'history-item-success' : 'history-item-fail';
      const statusText = r.success ? '✅成功' : '❌失败';
      const errorInfo = r.error ? ` - ${r.error}` : '';
      html += `<div class="history-item">`;
      html += `<span class="history-item-time">${time}</span>`;
      html += `<span class="${statusClass}">${statusText}</span>`;
      html += `<span class="history-item-detail">${name} 第${r.periodSort}讲 → ${r.newDate} ${r.newTime}${errorInfo}</span>`;
      html += `</div>`;
    }
    html += `</div>`;
  }

  elHistoryContainer.innerHTML = html;
}
...
let currentToken = ''; // 当前提取到的 JWT

/**
 * 从当前活动标签页的 Cookie 中提取 authorization-token
 */
async function extractTokenFromCookie() {
  try {
    ...
    if (cookie && cookie.value) {
      ...
      // 校验JWT格式
      if (/^eyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(token)) {
        currentToken = token;
        return { token, found: true };
      }
    }
  } catch (e) {
    console.error('[调课助手] 读取Cookie失败:', e);
  }
...
function updateTokenUI(result) {
  if (result.found && result.token) {
    ...
    if (payload && payload.exp) {
      ...
      if (!isExpired) {
        const hours = Math.floor((payload.exp - now) / 3600);
        remainingStr = hours >= 1 ? `剩余 ${hours} 小时` : `剩余 ${Math.floor((payload.exp - now) / 60)} 分钟`;
      }
    }

    if (isExpired) {
      elTokenStatusCard.className = 'token-card token-card-expired';
      elTokenStatusCard.innerHTML = `
        <div class="token-card-icon">⚠️</div>
        <div class="token-card-text">Token 已过期，请重新登录调课后台</div>
      `;
      elBtnCopyToken.disabled = true;
      hideSection(elTokenDetail);
    } else {
      elTokenStatusCard.className = 'token-card token-card-valid';
      elTokenStatusCard.innerHTML = `
        <div class="token-card-icon">✅</div>
        <div class="token-card-text">Token 有效 ${remainingStr ? '（' + remainingStr + '）' : ''}</div>
      `;
      elBtnCopyToken.disabled = false;

      // 显示详情
      showSection(elTokenDetail);
      const tokenStart = result.token.substring(0, 20);
      const tokenEnd = result.token.substring(result.token.length - 10);
      elTokenPreview.textContent = `${tokenStart}...${tokenEnd}`;
      elTokenPreview.title = '点击复制完整Token';
      elTokenExpire.textContent = expireStr;
      elTokenUser.textContent = payload?.sub || payload?.name || payload?.preferred_username || '-';
    }
  } else {
    currentToken = '';
    elTokenStatusCard.className = 'token-card token-card-empty';
    elTokenStatusCard.innerHTML = `
      <div class="token-card-icon">❌</div>
      <div class="token-card-text">未检测到 Token，请先登录调课后台</div>
    `;
    elBtnCopyToken.disabled = true;
    hideSection(elTokenDetail);
  }
...
async function copyTokenToClipboard() {
  ...
  try {
    ...
    elBtnCopyToken.textContent = '✅ 已复制！';
    elBtnCopyToken.classList.add('btn-copied');
    setTimeout(() => {
      elBtnCopyToken.textContent = '📋 复制 Token';
      elBtnCopyToken.classList.remove('btn-copied');
    }, 2000);
  } catch (e) {
...
async function refreshToken() {
  elTokenStatusCard.className = 'token-card token-card-checking';
  elTokenStatusCard.innerHTML = `
    <div class="token-card-icon">🔄</div>
    <div class="token-card-text">正在检测 Token...</div>
  `;
  hideSection(elTokenDetail);
  elBtnCopyToken.disabled = true;

  const result = await extractTokenFromCookie();
  updateTokenUI(result);
}
...
// Tab 切换
elTabToken.addEventListener('click', () => {
  elTabToken.classList.add('tab-active');
  elTabMain.classList.remove('tab-active');
  elTabHistory.classList.remove('tab-active');
  elPanelToken.classList.add('tab-panel-active');
  elPanelMain.classList.remove('tab-panel-active');
  elPanelHistory.classList.remove('tab-panel-active');
  refreshToken(); // 切换到Token页时自动刷新
});

elTabMain.addEventListener('click', () => {
  elTabMain.classList.add('tab-active');
  elTabToken.classList.remove('tab-active');
  elTabHistory.classList.remove('tab-active');
  elPanelMain.classList.add('tab-panel-active');
  elPanelToken.classList.remove('tab-panel-active');
  elPanelHistory.classList.remove('tab-panel-active');
});

elTabHistory.addEventListener('click', () => {
  elTabHistory.classList.add('tab-active');
  elTabToken.classList.remove('tab-active');
  elTabMain.classList.remove('tab-active');
  elPanelHistory.classList.add('tab-panel-active');
  elPanelToken.classList.remove('tab-panel-active');
  elPanelMain.classList.remove('tab-panel-active');
  renderHistory(); // 切换到历史时刷新
});

// 清空历史记录
elBtnClearHistory.addEventListener('click', () => {
  if (confirm('确定清空所有历史调课记录？此操作不可恢复。')) {
    clearHistory();
  }
});

// V4 Token 按钮
elBtnCopyToken.addEventListener('click', copyTokenToClipboard);
elBtnRefreshToken.addEventListener('click', refreshToken);
