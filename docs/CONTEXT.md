...
## 当前状态
- **项目版本**：v2.2.132（壳扩展工作台层级，本地未部署）
- **最后更新**：2026-06-29
- **最新构建**：插件工作箱（壳）v2.2.132，学习报告 v5.3.1，调课助手 v4.5.4，每日看板 v2.2.124，批量录入 v1.1.0

## 归属于该项目的完整插件清单

| # | 插件名称 | 类型 | 版本 | 目录位置 | 状态 |
|---|---------|------|------|---------|------|
| 1 | 插件工具箱（壳） | 核心插件 | v2.2.132 | `plugins/toolbox/` | 🔧修复中 |
| 2 | 学习报告批量分析 | 嵌入+独立双版 | v5.3.1 | `plugins/report/` + `modules/report/` | ✅稳定 |
| 3 | 页面表格提取 | 嵌入+独立双版 | v1.0.2 | `plugins/dingtalk/` + `modules/dingtalk/` | ✅稳定 |
| 4 | 调课助手 | 嵌入模块 | v4.5.4 | `plugins/toolbox/modules/tiaoke/` | ✅稳定 |
| 5 | 课程排期热力图 | 嵌入模块 | v1.0.0 | `plugins/toolbox/modules/heatmap/` | ✅稳定 |
| 6 | 每日工作看板 | 嵌入模块 | v2.2.124 | `plugins/toolbox/modules/dailyboard/` | ✅稳定 |
...
**Phase12（v2.2.70~v2.2.132）**：🆕 每日工作看板功能爆发——不专注率主动提醒(v2.2.70)、企微异常推送(v2.2.123)、扫描历史弹窗(v2.2.124)；调课助手课程匹配大修(v4.5.0~v4.5.4)——全学科排课+双层过滤+学科映射+学期提取+改约支持+失败汇总；模块缓存版本戳修复(v2.2.132)
...
- **全学科排课**（v4.5.0）：数学/语文/英语/物理/化学 → 思维/人文/演说/科学/实验，双层过滤禁止全科/S班
- **课程匹配两步走**（v4.5.2~v4.5.3）：学科映射 + 学期关键词（暑假/秋季/寒假/春季）精准定位，支持改约
- **失败汇总**（v4.5.4）：执行完成后末尾追加红色「失败详情」块，清晰列出失败条目
- 3 并发批量执行
- 学员信息簿（Excel 导入）
- 历史记录
- Shadow DOM 隔离

### 每日工作看板（嵌入模块）— v2.2.124 ✅稳定

- 爱芯后台页面内嵌看板，实时监控学生上课状态
- **不专注率主动提醒**（v2.2.70）：铃铛开关 + 每20分钟扫描 + ≥40%桌面通知（requireInteraction不自动消失）
- **企微推送**（v2.2.123）：不专注率异常自动推送到爱芯小助手，独立开关+测试按钮，按课节去重
- **扫描历史弹窗**（v2.2.124）：点击状态栏"🔔 监控中..."查看最近10次扫描记录
- 课节视角（日期范围 + 关键字过滤 + 列头排序）
- 学情绑定表（姓名/备注名/ID/手机号，chrome.storage.local 永久缓存）
